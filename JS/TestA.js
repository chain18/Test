...
function isRoomAvailable(room, startTime, endTime, data) {
  for (let i = 0; i < data.length; i++) {
    if (
      (data[i].roomId === room &&
        startTime >= data[i].startTime &&
        startTime < data[i].endTime) ||
      (endTime > data[i].startTime && endTime <= data[i].endTime) ||
      (startTime <= data[i].startTime && endTime >= data[i].endTime)
    ) {
      return false;
    }
  }
...
let roomId = "A101";
let startTime = "2019-09-28 15:00:00";
let endTime = "2019-09-28 16:00:00";

console.log(isRoomAvailable(roomId, startTime, endTime, data));
...
//   preferEndTime
// ) => {
//   const resvStartDt = new Date(reservedStartTime);
//   const resvEndDt = new Date(reservedEndTime);
//   const prefStartDt = new Date(preferStartTime);
//   const prefEndDt = new Date(preferEndTime);
//   if (prefStartDt >= resvStartDt) {
//     return prefStartDt <= resvEndDt;
//   } else {
//     return prefEndDt >= resvStartDt;
//   }
// };
...
//         isOverlapped(booking.startTime, booking.endTime, startTime, endTime)
//     ).length === 0
//   );
// };
